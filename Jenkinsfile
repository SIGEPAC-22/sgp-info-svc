    ...
    stage('RUN DB DEV') {
      ...
      steps {
        script {
          ...
          docker run --rm -v $WORKSPACE/sql:/flyway/sql -v $WORKSPACE/sql:/flywaydev/conf flyway/flyway:8.5.1 -user=$DB_CREDS_DEV_USR -password=$DB_CREDS_DEV_PSW migrate
          docker run --rm -v $WORKSPACE/sql:/flyway/sql -v $WORKSPACE/sql:/flywaydev/conf flyway/flyway:8.5.1 -user=$DB_CREDS_DEV_USR -password=$DB_CREDS_DEV_PSW validate
          docker run --rm -v $WORKSPACE/sql:/flyway/sql -v $WORKSPACE/sql:/flywaydev/conf flyway/flyway:8.5.1 -user=$DB_CREDS_DEV_USR -password=$DB_CREDS_DEV_PSW info
	  '''
        }
      }
    }
