...
type GetInfoPatientFileRepo struct {
	db  *sql.DB
	log log.Logger
}

func NewGetInfoPatientFileRepo(db *sql.DB, log log.Logger) *GetInfoPatientFileRepo {
	return &GetInfoPatientFileRepo{db: db, log: log}
}

func (g *GetInfoPatientFileRepo) GetPatientFileRepository(ctx context.Context) ([]getInfoPatientFile.GetInfoPatientFileResponse, error) {

	config := goconfig.NewConfig("./application.yaml", goconfig.Yaml)

	rows, errDB := g.db.QueryContext(ctx, "SELECT pat.pat_id_patient, pfl.pfl_id_patient_file, CONCAT_WS(' ',pat.pat_first_name, pat.pat_second_name, pat.pat_first_last_name, pat.pat_second_last_name) AS Fullname, \ndct.dct_document_type_name,pat.pat_document_number, pat.pat_cellphone_number, pat.pat_reponsible_family, pat.pat_responsible_family_phone_number,\nsex.spt_gender_type, pfl.pfl_pregnant, spt.spt_name_state_patient, pfl.pfl_admission_date, pfl.pfl_high_date, pfl.pfl_low_date\nFROM pat_patient AS pat\nINNER JOIN pfl_patient_file AS pfl ON pfl.pfl_id_patient = pat.pat_id_patient\nINNER JOIN sex_patient AS sex ON pat.pat_id_patient_sex = sex.spt_id_sex\nINNER JOIN spt_state_patient AS spt ON pfl.pfl_id_state_patient = spt.spt_id_state_patient\nINNER JOIN dct_document_type AS dct ON pat.pat_id_document_type = dct.dct_id_document_type;")
	if errDB != nil {
		g.log.Log("Error while trying to get information for PatientFile", constants.UUID, ctx.Value(constants.UUID))
		return []getInfoPatientFile.GetInfoPatientFileResponse{}, errDB
	}
	defer rows.Close()
	var resp []getInfoPatientFile.GetInfoPatientFileResponse
	for rows.Next() {
		var respDB SqlGetInfoPatientFile
		if err := rows.Scan(&respDB.IdPatient, &respDB.IdPatientFile, &respDB.FullName, &respDB.DocumentType, &respDB.DocumentNumber, &respDB.CellphoneNumber, &respDB.ResponsibleFamily, &respDB.ResponsibleFamilyPhoneNumber, &respDB.Sex, &respDB.Pregnant, &respDB.StatePatient, &respDB.AdmissionDate, &respDB.HighDate, &respDB.LowDate); err != nil {
		...
		resp = append(resp, getInfoPatientFile.GetInfoPatientFileResponse{
			...
			Sex:                          respDB.Sex,
			Pregnant:                     transformerPregnant(respDB.Pregnant),
...
func (g *GetInfoPatientFileRepo) GetComorbidityPatient(ctx context.Context, idPatientFile int) ([]string, error) {
	...
	if resp == nil {
		resp = append(resp, config.GetString("app-PropertiesInfoPatientFile.messageNotAvailable"))
	}
	return resp, nil
}

func (g *GetInfoPatientFileRepo) GetSymptomPatient(ctx context.Context, idPatientFile int) ([]string, error) {
...
func transformerPregnant(pregnant bool) string {

	config := goconfig.NewConfig("./application.yaml", goconfig.Yaml)

	if pregnant {
		return config.GetString("app-PropertiesInfoPatientFile.pregnantTrue")
	} else {
		return config.GetString("app-PropertiesInfoPatientFile.pregnantFalse")
	}
}
